(() => {
  ...
  function generateResizedSvg(svgEl, targetWidthPx, targetHeightPx, unit, options = {}) {
    ...
    const labelOffset = Math.max(fontSize * 0.5, strokeWidth * 3);
    ...
    const leftMargin = includeDimensions
      ? Math.max(marginBase, dimOffset + tickSize + labelOffset + fontSize * 0.5)
      : marginBase;
    ...
    const dimensionLines = includeDimensions
      ...
      <g data-generated-by="dimension-overlay" fill="none" stroke="${dimensionColors.stroke}" stroke-width="${strokeWidth}" stroke-linecap="round">
        <line x1="${viewBox.minX}" y1="${horizontalY}" x2="${viewBox.minX + viewBox.width}" y2="${horizontalY}" marker-start="url(#${markerIdBase}-start)" marker-end="url(#${markerIdBase}-end)" stroke-dasharray="${Math.max((viewBox.width - fontSize * 1.2) / 2, 0)} ${Math.max(fontSize * 1.2, 0.1)}"></line>
        <line x1="${verticalX}" y1="${viewBox.minY}" x2="${verticalX}" y2="${viewBox.minY + viewBox.height}" marker-start="url(#${markerIdBase}-start)" marker-end="url(#${markerIdBase}-end)" stroke-dasharray="${Math.max((viewBox.height - fontSize * 1.2) / 2, 0)} ${Math.max(fontSize * 1.2, 0.1)}"></line>
    ...
    const labels = includeDimensions
      ...
      <g data-generated-by="dimension-overlay" fill="${dimensionColors.text}" font-size="${fontSize}" font-weight="600" font-family="'Segoe UI', 'Hiragino Sans', 'Yu Gothic', sans-serif">
        ...
        <text x="${verticalLabelX}" y="${viewBox.minY + viewBox.height / 2}" text-anchor="end" dominant-baseline="middle" transform="rotate(-90 ${verticalLabelX} ${viewBox.minY + viewBox.height / 2})">${heightLabelText}</text>
      </g>`
      : '';
