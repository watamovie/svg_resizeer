(() => {
  ...
  function generateResizedSvg(svgEl, targetWidthPx, targetHeightPx, unit, options = {}) {
    ...
    const strokeWidth = Math.max(scaleRef * 0.003, 0.6);
    const baseFontSize = Math.max(scaleRef * 0.058, 11);
    ...
    const textOffset = Math.max(fontSize * 0.65, strokeWidth * 5, tickSize * 0.7);
    ...
    const defs = includeDimensions
      ...
      <defs data-generated-by="dimension-overlay">
        <marker id="${markerIdBase}-start" markerWidth="12" markerHeight="10" refX="12" refY="5" orient="auto" markerUnits="strokeWidth" viewBox="0 0 12 10">
          <path d="M12 5L0 10V0L12 5Z" fill="${dimensionColors.stroke}"></path>
        </marker>
        <marker id="${markerIdBase}-end" markerWidth="12" markerHeight="10" refX="0" refY="5" orient="auto" markerUnits="strokeWidth" viewBox="0 0 12 10">
          <path d="M0 5L12 10V0L0 5Z" fill="${dimensionColors.stroke}"></path>
        </marker>
      </defs>`
      : '';
    ...
    const dimensionLines = includeDimensions
      ? `
      <g data-generated-by="dimension-overlay" fill="none" stroke="${dimensionColors.stroke}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round">
        ...
        <line x1="${viewBox.minX}" y1="${viewBox.minY}" x2="${viewBox.minX}" y2="${horizontalY}" stroke-dasharray="none"></line>
        <line x1="${viewBox.minX + viewBox.width}" y1="${viewBox.minY}" x2="${viewBox.minX + viewBox.width}" y2="${horizontalY}" stroke-dasharray="none"></line>
        <line x1="${viewBox.minX}" y1="${viewBox.minY}" x2="${verticalX}" y2="${viewBox.minY}" stroke-dasharray="none"></line>
        <line x1="${viewBox.minX}" y1="${viewBox.minY + viewBox.height}" x2="${verticalX}" y2="${viewBox.minY + viewBox.height}" stroke-dasharray="none"></line>
    ...
    const labels = includeDimensions
      ? `
      <g data-generated-by="dimension-overlay" fill="${dimensionColors.text}" font-size="${fontSize}" font-weight="500" letter-spacing="0.02em" font-family="'Segoe UI', 'Hiragino Sans', 'Yu Gothic', sans-serif">
