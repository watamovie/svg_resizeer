(() => {
  ...
  function generateResizedSvg(svgEl, targetWidthPx, targetHeightPx, unit, options = {}) {
    ...
    const labelClearance = Math.max(fontSize * 0.35, strokeWidth * 4);
    const dimOffset = Math.max(marginBase * 0.6, tickSize + fontSize * 0.35);

    const baseBottomMargin = includeDimensions
      ? dimOffset + tickSize + labelClearance + fontSize * 0.6
      : marginBase;
    ...
    const leftMargin = includeDimensions
      ? Math.max(marginBase, dimOffset + tickSize + labelClearance + fontSize * 0.6)
      : marginBase;
