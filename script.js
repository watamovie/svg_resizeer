(() => {
  ...
  function generateResizedSvg(svgEl, targetWidthPx, targetHeightPx, unit, options = {}) {
    ...
    const extensionOvershoot = Math.max(strokeWidth * 4, tickSize * 0.35);
    const labelOffset = Math.max(fontSize * 0.85, tickSize * 0.6, strokeWidth * 6);
    ...
    const rightMargin = Math.max(marginBase, extensionOvershoot);
    const topMargin = Math.max(marginBase, extensionOvershoot);
    const leftMargin = Math.max(
      marginBase,
      dimOffset + extensionOvershoot + labelOffset + fontSize * 0.6
    );
    ...
    const defs = includeDimensions
      ...
      <defs data-generated-by="dimension-overlay">
        <marker id="${markerIdBase}" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse" markerUnits="strokeWidth">
          <path d="M0 0L10 5L0 10Z" fill="${dimensionColors.stroke}"></path>
        </marker>
      </defs>`
      : '';
    ...
    const dimensionLines = includeDimensions
      ...
      <g data-generated-by="dimension-overlay" fill="none" stroke="${dimensionColors.stroke}" stroke-width="${strokeWidth}" stroke-linecap="square">
        <line x1="${viewBox.minX}" y1="${horizontalY}" x2="${viewBox.minX + viewBox.width}" y2="${horizontalY}" marker-start="url(#${markerIdBase})" marker-end="url(#${markerIdBase})"></line>
        <line x1="${verticalX}" y1="${viewBox.minY}" x2="${verticalX}" y2="${viewBox.minY + viewBox.height}" marker-start="url(#${markerIdBase})" marker-end="url(#${markerIdBase})"></line>
        <line x1="${viewBox.minX}" y1="${viewBox.minY - extensionOvershoot}" x2="${viewBox.minX}" y2="${horizontalY + extensionOvershoot}"></line>
        <line x1="${viewBox.minX + viewBox.width}" y1="${viewBox.minY - extensionOvershoot}" x2="${viewBox.minX + viewBox.width}" y2="${horizontalY + extensionOvershoot}"></line>
